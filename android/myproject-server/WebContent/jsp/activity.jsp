...
function agree(id){
	var acti='${sessionScope.user.actiid}';
	if(acti.indexOf(id)>0){
		alert('您已参加过该活动，可选择取消参加');
		
	} else{
		alert('欢迎参加活动，恭喜您获得积分'+id);
		window.location ="/myproject/agreejoinActiServlet?actiid="+id;
...
function disagree(id){
	var acti='${sessionScope.user.actiid}';
	if(acti.indexOf(id)>0){
		alert('选择取消参加,还可以选择别的活动哦');
		window.location ="/myproject/canceljoinActiServlet?actiid="+id;
		
	} else{
		alert('您尚未参加过该活动，不可以取消参加哦');
...
<div class="main">
    ...
    <div id="main-content" class="box">
      ...
      <!----搜索活动项目开始----->
      <form name="searchActiKind" class="navbar-form navbar-right" method="post"
				action="/myproject/showActiBySnameServlet">
				
				  <input name="actiSname" id="actiSname" type="text" class="form-control" style="align:center;"
					placeholder="搜索运动类别">
					<button  class="btn btn-info" type="submit" class="btn">G0!</button>
		     </p>
			</form>
          <!----搜索活动项目结束-------->
          
            <form name="searchActiByAname" class="navbar-form navbar-right" method="post"
				action="/myproject/showActiByAnameServlet">
				<p>
				  <input name="actiname" id="actiname" type="text" class="form-control" style="align:center;"
					placeholder="搜索活动名称">
					<button  class="btn btn-info" type="submit" class="btn">G0!</button>
		     </p>
			</form>
      <form  name="form1" method="post" action="">  
        ...
        <select name="listorder" id="排列方式">
          <option value="默认方式">默认方式</option>
          <option value="按时间顺序">按时间顺序</option>
          <option value="按人气">按人气</option>       
        </select>
      </form>
      <!---翻页必用------->
...
<%
						IActivityinvitationDao activityinvitationDao=new ActivityinvitationDaoImpl();
							List<Activityinvitation> lstacti=activityinvitationDao.selectAllActivityinvitation();
							for(Activityinvitation acti:lstacti){
							int userid=acti.getUserid();
							int sportid=acti.getSportid();
							...
							Sport sport=sportDao.selectBySportid(sportid);
					%>
<div class="list-item box">					
<!-- img -->
<div class="img left">
<p><img src="/myproject/jsp/image/02.jpg" alt="头像" title="头像"/></p>
<p>
<h5> 发帖人昵称：<div class="username"><%=person.getNickname()%></div></h5>
</p>
</div>
<!-- data -->
<div class="block right">
<p class="date">发帖时间：<%=acti.getActisettime()%></p>
<p></p>
<p class="title">运动类别：[<%=sport.getSportname()%>]</p>
<p class="title">活动名称：[<%=acti.getActiname()%>]</p>
 <p></p>
<p class="desc">活动内容：<%=acti.getActicont()%></p>                           <p></p>
 <p class="like">活动地点：<%=acti.getActiplace()%></p>
 <p></p>
<p class="desc">活动开始时间：<%=acti.getActibegtime()%>       |   
活动结束时间：<%=acti.getActiendtime()%></p>
 <p></p>
<div align="right"> 
<p class="desc">已有<%=acti.getActiusercount()%>人参加|赞（<%=acti.getActidianzan()%>)
<a  class="btn" href="/myproject/dianZanActiServlet?userid=${sessionScope.person.regid}&actiid=<%=acti.getActiid()%>"  onClick="javascript:alert('点赞');">点赞 </a>|
<a class="btn" id="agree" name="agree"   value="1" onClick="javascript:agree('<%=acti.getActiid()%>');">同意参加 </a>|
<a class="btn" id="disagree" name="disagree"   role="button"  onClick="javascript:disagree('<%=acti.getActiid()%>');">取消参加 </a>
										</div>
								</div>
							</div>
							    <%
...
<form id="post" name="post" method="post" action="/myproject/publishActivityServlet">
      ...
      <input type="hidden" id="userid" name="userid" value="${sessionScope.person.regid}"/>
   	    ...
   	    <input name="actibegtime" id="begtime" type="datetime" class="form-control" style="width:200px;height:30px;" placeholder="xxx-xx-xx xx:xx">
   	    ...
   	    <label>
        <button class="btn-info"  name="publish" id="publish" type="submit" onClick="javascript:alert('发起活动成功，恭喜获得积分');">提交</button>
<br>
<br>
        </label>
  ...
  <div class="clearfix"></div>
		<div>
			<a class="btn btn-info"
				...
				<div id="joinedacti" name="joinedacti" style="display: none;">
					<table width="93%" border="0" align="center">
						...
						%>
						<%
							List<Actihuo> lstActi=activityBiz.JoinedActivity(uid);
										if(lstActi!=null){
										for(Actihuo acti:lstActi){
											int id=acti.getActiid();
						%>
						<tr>
							<td height="35" align="center" bgcolor="#CCC"><%=acti.getNickname()%></td>
						  <td align="center" bgcolor="#CCC" ><%=acti.getSportname()%></td>
						  <td align="center" bgcolor="#CCC"><a href="/myproject/showActiByAnameServlet?actiname=>"><%=acti.getActiname()%></a></td>
						</tr>
					<%
						} }
					%>
	     ...
	     <table width="93%" border="0" align="center">
		          ...
		          %>
			      <tr>
			        <td height="89" align="center" bgcolor="#CCC"><%=activity.getNickname()%></td>
			        <td align="center" bgcolor="#CCC"><%=activity.getSportname()%></td>
			        <td align="center" bgcolor="#CCC"><%=activity.getActiname()%></td>
		          </tr>
			     <%
			     	}}
			     %>
	     ...
	     <p>&nbsp;</p>
                <table width="93%" border="0" align="center">
                  ...
                  <tr>
                   ...
                   %>
                  <tr>
                    <td height="78" align="center" bgcolor="#CCC"><%=lstacti3.get(i).getNickname() %></td>
                    <td align="center" bgcolor="#CCC"><%=lstacti3.get(i).getSportname()  %></td>
                    <td align="center" bgcolor="#CCC"><%=lstacti3.get(i).getActiname() %></td>
                    <td align="center" bgcolor="#CCC"><%=lstacti3.get(i).getActiusercount() %></td>
                  </tr>
                 <%} }%>
                </table>
