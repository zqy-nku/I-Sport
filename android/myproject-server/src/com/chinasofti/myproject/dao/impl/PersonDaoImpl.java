...
public class PersonDaoImpl implements IPersonDao {
	...
	//删除数据部分
		...
		public Person selectByObject(String account, String password) {
			...
			try {
				if (resultSet.next()) {
					...
					person1.setRegid(resultSet.getInt(1));
					person1.setAccount(resultSet.getString(2));
					person1.setPassword(resultSet.getString(3));
					person1.setNickname(resultSet.getString(4));
					person1.setRegtime(resultSet.getString(5));
					person1.setRegaddr(resultSet.getString(6));
					person1.setReggender(resultSet.getString(7));
					// 步骤7：返回对象
					return person1;
				} else {
					return null;
		...
		@Override
		public Person findByCondition(Person person) {
			// TODO Auto-generated method stub
			...
			try {
				if(resultSet.next()){
					Person p = new Person();
					p.setRegid(resultSet.getInt(1));
					p.setAccount(resultSet.getString(2));
					p.setPassword(resultSet.getString(3));
					p.setNickname(resultSet.getString(4));
					p.setRegtime(resultSet.getString(5));
					p.setRegaddr(resultSet.getString(6));
					p.setReggender(resultSet.getString(7));
					return p;
				}else{
					return null;
				}
