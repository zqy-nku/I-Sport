package com.chinasofti.myproject.servlet;

import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.chinasofti.myproject.biz.ICoachBiz;
import com.chinasofti.myproject.biz.IVenueBiz;
import com.chinasofti.myproject.biz.impl.CoachBizImpl;
import com.chinasofti.myproject.biz.impl.VenueBizImpl;
import com.chinasofti.myproject.po.Coach;
import com.chinasofti.myproject.po.Venue;

/**
 * Servlet implementation class showCoachVenueServlet
 */
public class showCoachVenueServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public showCoachVenueServlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doPost(request,response);
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		System.out.println("here");
		request.setCharacterEncoding("UTF-8");
		int sportid=Integer.parseInt(request.getParameter("sportid"));
		System.out.println(sportid);
		ICoachBiz coachBiz=new CoachBizImpl();
		List<Coach> lstCoach = new ArrayList<Coach>();
		lstCoach=coachBiz.selectBySportid(sportid);
		
		IVenueBiz venueBiz=new VenueBizImpl();
		List<Venue> lstVenue=new ArrayList<Venue>();
		lstVenue=venueBiz.selectBySportid(sportid);
		if ((lstCoach==null)||(lstVenue==null)) {
			response.sendRedirect("jsp/selectWrong.html");
		}else{
		request.setAttribute("sportid", sportid);
		request.setAttribute("lstCoach", lstCoach);
		request.setAttribute("lstVenue", lstVenue);
		// 步骤4：使用请求转发模式进行页面跳转
		RequestDispatcher dispatcher = request.getRequestDispatcher("jsp/coach-venue.jsp");
		dispatcher.forward(request, response);
		}
		
		
		
		
		
		
		
	}

}
