...
public class JoinedActivity extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 设置标题栏
	   	...
	   	//setContentView(R.layout.joined_acti);
	   	getListView();
		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE,
				R.layout.titlebar_joinedacti);
		// 取得sharedpreferene数据
		...
		this.goback=(Button)super.findViewById(R.id.joinedacti_return);
		// 设置适配器
	...
	// 获取数据
		private List<Map<String, ?>> fetchData() {
			...
			lstNameValuePairs.add(new BasicNameValuePair("user_data", user_data));
			// 步骤2-2：调用方式实现请求的发送
			String response = WebAccessUtils.httpRequest("JoinedActiServlet", lstNameValuePairs);				
			System.out.println("response="+response);
			// 步骤3：处理JSON数据
			// 步骤4-3：设置一个全新的类型Type
		...
		// 步骤4-6：使用循环遍历集合对象
				for (Actihuo acti: lstActi) {
					...
					item01.put("userid", JoinedActivity.this.userid);
	...
	private class GobackListener implements OnClickListener{
		...
		public void onClick(View v) {
			...
			intent.setClass(JoinedActivity.this, MainActivity.class);
