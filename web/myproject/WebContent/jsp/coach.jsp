...
<%@page import="com.chinasofti.myproject.dao.impl.CoachcommentDaoImpl"%>
<%@page import="com.chinasofti.myproject.po.Coachcomment"%>
<%@page import="com.chinasofti.myproject.po.Coach"%>
<%@page import="com.chinasofti.myproject.dao.ICoachcommentDao"%>
	...
	<div class="h_menu">
	  ...
	  <script src="js/menu.js" type="text/javascript"></script>
		...
		<div class="col-md-6 span1_of_2"><!-- start span1_of_3 style2 -->
                           ...
                           <a class="btn btn-sm btn-defualt" href="/myproject/showCoachVenueServlet?sportid=${requestScope.sportid}">返上一页面</a>   
							<div class="list-item box" style="margin-left:0px; height:250px;">					
								...
								<div class="img left"  style="width:200px; height:0px;">
									<a><img src="/myproject/jsp/img/${coach.coaimage}" style="height:200px;"></a>
								</div>
								...
								<p class="desc">
								  ...
								  <table width="89%" height="128" border="0">
								    <tr>
								      ...
								      <th width="50%" scope="row">姓名：${coach.coaname}</th>
								      <td width="23%">性别：${coach.coagender}</td>
								      <td width="1%">&nbsp;</td>
							        </tr>
								    <tr>
								      <th scope="row">&nbsp;</th>
								      <th scope="row">&nbsp;</th>
								      <th scope="row">&nbsp;</th>
								      <th scope="row">联系方式：${coach.coaphon}</th>
								      <td>详细信息：${coach.coacontext}</td>
								      <td>&nbsp;</td>
							        </tr>
							    </table>
								  <p class="like"></p>
                             ...
                             <%
			ICoachcommentDao coachcommentDao=new CoachcommentDaoImpl();
            Coach coach=(Coach)request.getAttribute("coach");
            int coachid=coach.getCoachid();
            List<Coachcomment> lstCoachcomments=coachcommentDao.selectByCoaid(coachid);
            %>              
                      <h5>最新评论:共有<%=lstCoachcomments.size() %>评论</h5>
                  ...
                  <%
			for(Coachcomment cc:lstCoachcomments){
			...
			%>
               ...
               <!-- item 1 -->
							<div class="list-item box">					
								...
								<!-- data -->
								<div class="block right">
									<p class="date"><%=cc.getCoacomstar() %></p>
									<p class="title"><%=peruser.getNickname() %></p>
									<p class="desc"><%=cc.getCoacomresid() %></p>
									<p class="like">评分：   <%=cc.getCoacomcont() %></p>
								</div>
							</div>
							  <%} %>
            ...
            <hr> 
           <form name="comment" id="comment" method="post" action="/myproject/commentCoachServlet?coachid=${coach.coachid}&userid=${sessionScope.person.regid}">  
            <h5>发表评论：</h5>
           <div class="team-members">
								<div class="col-md-3">
									<div class="team-member text-center">
										<textarea name="coacomcont" id="coacomcont" cols="46.5" rows="3"
						placeholder="请输入评论......" class="alert-info"></textarea>
										<h3><input name="coacomstar" id="coacomstar" cols="45"type="text" placeholder="请评分（0~10）分..."
								class="alert-info"></h3>
										<span><div align="center"><button class="btn btn-info" type="submit" name="cc" id="cc">提交评论</button></div></span>
