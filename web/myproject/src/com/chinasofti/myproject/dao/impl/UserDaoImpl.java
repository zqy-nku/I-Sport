...
public class UserDaoImpl implements IUserDao {
	...
	public int updateUser(User user) {
		...
		String strSQL = "update user set regid=?, spordec=?,spornum=?,rank=?,integ=?,sportlabel=?,isaccepmsg=?,collestatusid=?,colletreasid=?,userpho=?,userstatus=?,mark=?,actiid=? where userid=?";
		Object[] params = new Object[] { regid, spordec, spornum, rank, integ,
				sportlabel, isaccepmsg, collestatusid, colletreasid, userpho,
				userstatus, mark,actiid, userid };
		int affectedRows = dbUtils.execOthers(conn, strSQL, params);
		if (affectedRows > 0) {
			TransactionManager.commit();
		} else {
