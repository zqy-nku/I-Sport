...
public class SelectTreasureServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//加载tips结束
		//判断jsp页面是否有选择运动项目的选择
		List<Sporttreasure> lstSporttreasure;
		int sportid = Integer.parseInt(request.getParameter("sportid"));
		System.out.println("select="+sportid);
		if (sportid == 0) {
			 lstSporttreasure = sporttreasureBiz.findAll();
		} else {
			 lstSporttreasure = sporttreasureBiz.findSportBySportid(sportid);	
