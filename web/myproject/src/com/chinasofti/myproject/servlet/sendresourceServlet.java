...
import com.chinasofti.myproject.biz.impl.ResourceBizImpl;
...
/**
 * Servlet implementation class sendresource
 */

public class sendresourceServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public sendresourceServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int userid=Integer.parseInt(request.getParameter("userid"));
				...
				System.out.println("[SingleFileUploadServlet] 文件上传地址" + fileUploadPath);
				...
				System.out.println("[SingleFileUploadServlet] 缓存地址" + fileUploadTempPath.getPath());
				
				// 锟斤拷锟斤拷2锟斤拷锟叫断�锟角凤拷锟斤拷锟较达拷要锟斤拷
					...
					try {
						...
						for (FileItem fileItem : fileItems) {
							if(fileItem.isFormField()){
								...
								if("resdesc".equalsIgnoreCase(name)){
									String value = fileItem.getString();
									value = new String(value.getBytes("iso8859-1"),"UTF-8");
									resource.setResdesc(value);
									System.out.println("测试-相册介绍：> " + value);
								}	
							    ...
							    resource.setReslocation(fileName);
								String[] allowedTypes = new String[]{".jpg", ".jpeg", ".png", ".bmp","wma"};
					    ...
					    resource.setRestime(restime);
						IResourceBiz resourceBiz = new ResourceBizImpl();
						boolean flag=resourceBiz.sendphoto(resource);
						System.out.println(flag+"1");
						//
						if(flag){
							System.out.println(flag+"2");
						    response.sendRedirect("jsp/photo.jsp");
						}else
